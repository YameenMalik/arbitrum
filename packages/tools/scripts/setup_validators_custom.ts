import { findEnv } from '../utils/find-env';
...
async function initializeWallets(count: number, createNewNodesWallets: boolean): Promise<ethers.Wallet[]> {
  ...
  if (createNewNodesWallets == true) {
    ...
    for(let index in files){
      ...
      wallets.push(await ethers.Wallet.fromEncryptedJson(json, walletsPass));
    }
  }
  return wallets
...
async function setupValidators(
  ...
  const { rollupAddress, inboxAddress } = await setupRollup(wallets[0].address)
